C/*!
*    \file dcdflib.f
*    \brief Library of Fortran Routines for statistical distributions
*
*
...
*variables.
*
*If you want to use functions in this library, just define the functions
*and use \em standard rules for calling Fortran from C.
*
*\verbinclude dcdflib-doc.txt
*/
C
C                                     DCDFLIB
C 
...
C 
C 
C  This work was supported by grant CA-16672 from the
...
C     Transactions implies that unrestricted use of the algorithm within
C     a computer is permissible.  General permission to copy and
C     distribute the algorithm without fee is granted provided that the
C     copies are not made or distributed for direct commercial
C     advantage.  The ACM copyright notice and the title of the
C     publication and its date appear, and notice is given that copying
C     is by permission of the Association for Computing Machinery.  To
C     copy otherwise, or to republish, requires a fee and/or specific
C     permission.
C
C     Krogh, F.  Algorithms Policy.  ACM Tran.  Math.  Softw.  13(1987),
C     183-186.
C 
C     We place the DCDFLIB code that we have written in the public
C     domain.
C 
C                                  NO WARRANTY
C      
...
C     SHOULD THIS PROGRAM PROVE DEFECTIVE, YOU ASSUME THE COST OF ALL
C     NECESSARY SERVICING, REPAIR OR CORRECTION.
C      
C     IN NO EVENT SHALL THE UNIVERSITY OF TEXAS OR ANY OF ITS COMPONENT
C     INSTITUTIONS INCLUDING M. D.  ANDERSON HOSPITAL BE LIABLE TO YOU
C     FOR DAMAGES, INCLUDING ANY LOST PROFITS, LOST MONIES, OR OTHER
C     SPECIAL, INCIDENTAL OR CONSEQUENTIAL DAMAGES ARISING OUT OF THE
C     USE OR INABILITY TO USE (INCLUDING BUT NOT LIMITED TO LOSS OF DATA
C     OR DATA OR ITS ANALYSIS BEING RENDERED INACCURATE OR LOSSES
C     SUSTAINED BY THIRD PARTIES) THE PROGRAM.
C      
...
C 
C     WHICH and STATUS are INTEGER, all other arguments are DOUBLE
C     PRECISION.
C 
C     <name> is a one to three character name identifying the
C     distribution.  WHICH is an input integer value that identifies
C     what parameter value is to be calculated from the values of the
C     other parameters.
C 
C     P is always the cdf evaluated at X, Q is always the compliment of
C     the cdf evaluated at X, i.e.  1-P, and X is always the value at
C     which the cdf is evaluated.  The auxiliary parameters,
C     <parameters>, of the distribution differ by distribution.
C 
C     If WHICH is 1, P and Q are to be calculated, i.e., the cdf; if
C     WHICH is 2, X is to be calculated, i.e., the inverse cdf.  The
C     value of one auxiliary parameter in <parameters> can also be the
C     value calculated.
C 
...
C     range (see below).  Parameters are counted with WHICH being the
C     first in these return values.
C 
C     A STATUS value of 1 indicates that the desired answer was
C     apparently lower than the lower bound on the search interval.  A
C     return code of 2 indicates that the answer was apparently higher
C     than the upper bound on the search interval.  A return code of 3
C     indicates that P and Q did not sum to 1. Other positive codes are
C     routine specific.
C 
...
C     returned as 1 or 2 then BOUND is returned as the lower or upper
C     bound on the search interval respectively.
C 
C                                 BOUNDS
C 
C     Below are the rules that we used in determining bounds on
C     quantities to be calculated.  Those who don't care can find a
C     summary of the bounds in dcdflib.chs.  Input bounds are checked
C     for legality of input.  The search range is the range of values
C     searched for an answer.
C 
C                              Input Bounds
C 
C     Bounds on input parameters are checked by the CDF* routines.
C     These bounds were set according to the following rules.
C 
C     P: If the domain of the cdf (X) extends to -infinity then P must
C     be greater than 0 otherwise P must be greater than or equal to 0.
C     P must always be less than or equal to 1.
C 
C     Q: If the domain of the cdf (X) extends to +infinity then Q must
C     be greater than 0 otherwise Q must be greater than or equal to 0.
C     Q must always be less than or equal to 1.
C 
C     Further, P and Q must sum to 1. The smaller of the two P and Q
C     will be used in calculations to increase accuracy
C 
C     X: If the domain is infinite in either the positive or negative
C     direction, no check is performed in that direction.  If the left
C     end of the domain is 0, then X is checked to assure
C     non-negativity.
C 
C     DF, SD, etc.: Some auxiliary parameters must be positive. The
C     lowest input values accepted for these parameters is 1E-300.
C 
C 
C                                 Search Bounds
C 
...
C     this value is the same endpoint of the search range.  If the
C     domain is open at some finite endpoint (which only occurs for 0 --
C     some parameters must be strictly positive) then the endpoint is
C     1E-300. If the domain is infinite in either direction then +/-
C     1E300 is used as the endpoint of the search range.
C 
...
C     distributions.  For example, the binomial and negative binomial
C     cdfs can be converted to a beta cdf.  This is how fractional
C     observations are handled.  The formula from Abramowitz and Stegun
C     for converting the cdfs is cited in the fdoc file.  (We think the
C     formula for the negative binomial in A&S is wrong, but there is a
C     correct one which we used.)
C 
C     The inverse normal and gamma are also taken from the references.
C     For all other parameters, a search is made for the value that
C     provides the desired P.  Initial values are chosen crudely for the
C     search (e.g., 5).  If the domain of the cdf for the parameter
C     being calculated is infinite, a step doubling strategy is used to
C     bound the desired value then the zero finder is employed to refine
C     the answer.  The zero finder attempts to obtain the answer
C     accurately to about eight decimal places.
C 
C 
C 
C  
...
C     TO GET-FUNCTION-VALUE
  ...
  280 CONTINUE
      if (i99999 .eq. 10) goto 10
      if (i99999 .eq. 20) goto 20
      if (i99999 .eq. 200) goto 200
      stop
      END
...
C      IF (iop-2) 280,290,300
      IF (iop-2 .lt. 0) goto 280
      IF (iop-2 .eq. 0) goto 290
      IF (iop-2 .gt. 0) goto 300
...
C      IF (iop-2) 340,350,360
      IF (iop-2 .lt. 0) goto 340
      IF (iop-2 .eq. 0) goto 350
      IF (iop-2 .gt. 0) goto 360
